...
export default class extends Controller {
  ...
  async loadFile() {
    ...
    try {
      ...
      if (wbEntry) {
        ...
        if (sheets.length > 0) {
          sheetName = sheets[0].getAttribute("name") || "Sheet1"
        }
      }
      ...
      this.resultSheetNameTarget.textContent = sheetName
      this.generateCsv()
  ...
  generateCsv() {
    ...
    this.csvOutputTarget.value = csv
    this.resultRowsTarget.textContent = this.rows.length
    this.resultColsTarget.textContent = this.rows.length > 0 ? Math.max(...this.rows.map(r => r.length)) : 0
  ...
  download() {
    const csv = this.csvOutputTarget.value
    if (!csv) return
  ...
  showError(msg) {
    this.previewTableTarget.innerHTML = `<p class="text-red-500 text-sm">${this.escapeHtml(msg)}</p>`
    this.previewTarget.classList.remove("hidden")
  }
