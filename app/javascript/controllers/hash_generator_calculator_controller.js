...
export default class extends Controller {
  static targets = [
    "input", "algorithm",
    "resultHash", "resultAlgorithm", "resultInputLength", "resultHashLength"
  ]
  ...
  async calculate() {
    ...
    const hash = await this.computeHash(text, algorithm)

    this.resultHashTarget.value = hash
    this.resultAlgorithmTarget.textContent = algorithm.toUpperCase()
    this.resultInputLengthTarget.textContent = text.length.toLocaleString()
    this.resultHashLengthTarget.textContent = hash.length + " chars"
  }

  async computeHash(text, algorithm) {
  ...
  clearResults() {
    this.resultHashTarget.value = ""
    this.resultAlgorithmTarget.textContent = "\u2014"
    this.resultInputLengthTarget.textContent = "\u2014"
    this.resultHashLengthTarget.textContent = "\u2014"
  }

  copy() {
    const text = this.resultHashTarget.value
    if (!text) return
