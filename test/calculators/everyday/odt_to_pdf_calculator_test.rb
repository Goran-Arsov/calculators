...
class Everyday::OdtToPdfCalculatorTest < ActiveSupport::TestCase
  # --- Happy path ---

  test "returns valid result with paragraph and word counts" do
    result = Everyday::OdtToPdfCalculator.new(
      paragraphs: [
        { text: "Hello world" },
  ...
  test "errors accessor returns empty array before call" do
    calc = Everyday::OdtToPdfCalculator.new(paragraphs: [ { text: "test" } ])
  ...
  test "error when paragraphs is nil" do
    result = Everyday::OdtToPdfCalculator.new(paragraphs: nil).call
  ...
  test "error when paragraphs is empty" do
    result = Everyday::OdtToPdfCalculator.new(paragraphs: []).call
    assert_equal false, result[:valid]
    assert result[:errors].any?
    assert_includes result[:errors], "No content provided"
  end
  ...
  test "error when paragraphs is not an array" do
    result = Everyday::OdtToPdfCalculator.new(paragraphs: "not an array").call
    assert_equal false, result[:valid]
    assert result[:errors].any?
